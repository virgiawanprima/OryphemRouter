...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("oryphemrouter");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "oryphemrouter-general-purpose",
      explore: "oryphemrouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.oryphemrouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.oryphemrouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.oryphemrouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# oryphemrouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.oryphemrouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("oryphemrouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.oryphemrouter-");
    expect(reset).not.toContain("oryphemrouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("oryphemrouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("oryphemrouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("oryphemrouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("oryphemrouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("oryphemrouter-plan");
