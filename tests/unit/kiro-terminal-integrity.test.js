...
describe("Kiro terminal integrity recovery", () => {
  ...
  it("surfaces retry HTTP failures as SSE after heartbeat commits headers", async () => {
    const errResp = () => new Response("unauthorized", {
      status: 401,
      statusText: "Unauthorized"
    });
    fetchMock
      .mockResolvedValueOnce(response([]))
      .mockResolvedValueOnce(errResp())
      .mockResolvedValueOnce(errResp())
      .mockResolvedValueOnce(errResp());
  ...
  it("bounds the retry HTTP error body", async () => {
    const errBody = `error-start-${"x".repeat(10_000)}-error-tail`;
    const errResp = () => new Response(errBody, {
      status: 401,
      statusText: "Unauthorized"
    });
    fetchMock
