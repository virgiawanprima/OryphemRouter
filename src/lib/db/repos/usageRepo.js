...
export async function saveRequestUsage(entry) {
  try {
    ...
    // ponytail: removed the ms-precision dedup — it collapsed distinct concurrent
    // requests sharing the same timestamp+attrs (data loss under load). Callers
    // guard double-logging at the source; add an explicit requestId dedup if needed.
    db.transaction(() => {
      db.run(
