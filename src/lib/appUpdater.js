...
// Collect PIDs of all oryphemrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from oryphemrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("oryphemrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("oryphemrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "oryphemrouter");
  }
  return path.join(os.homedir(), ".oryphemrouter");
}
...
// Resolve npx/oryphemrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx oryphemrouter` — works regardless of global bin path changes after npm i -g
