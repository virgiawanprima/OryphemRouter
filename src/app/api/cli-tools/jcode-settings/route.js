...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-oryphemrouter.env");
};
...
const hasoryphremrouterConfig = (config) => {
  ...
  if (providers["oryphemrouter"]) return true;
...
export async function POST(request) {
  try {
    ...
    config.providers["oryphemrouter"] = {
    ...
    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile oryphemrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["oryphemrouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "oryphemrouter configuration removed from jcode",
    });
  } catch (error) {
