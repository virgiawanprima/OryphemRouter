...
export async function POST(request) {
  try {
    const { baseUrl, apiKey, model, contextWindow, subagentModels } = await parseJson(request);
    ...
    const toml = applyGrokBuildConfig(await readConfigToml(), {
      ...
      apiKey: apiKey || "sk_oryphemrouter",
      model: selectedModel,
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "oryphemrouter model slots removed from Grok Build",
    });
  } catch (error) {
