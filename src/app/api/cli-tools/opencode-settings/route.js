...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["oryphemrouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("oryphemrouter/") ? config.model.replace(/^oryphemrouter\//, "") : null,
...
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_oryphemrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing oryphemrouter provider entry and its models
    const existingProvider = config.provider["oryphemrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["oryphemrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `oryphemrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `oryphemrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("oryphemrouter/")) {
        config.model = "";
      }
    }
...
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["oryphemrouter"]?.models) {
      delete config.provider["oryphemrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["oryphemrouter"].models).length === 0) {
        delete config.provider["oryphemrouter"];
        if (config.model?.startsWith("oryphemrouter/")) delete config.model;
      } else if (config.model === `oryphemrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["oryphemrouter"].models);
        config.model = `oryphemrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire oryphemrouter provider
      if (config.provider) delete config.provider["oryphemrouter"];
      if (config.model?.startsWith("oryphemrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("oryphemrouter/")) {
