...
// Read per-agent models.json and return current model id (without "oryphemrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["oryphemrouter"]?.models;
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["oryphemrouter"] = {
    baseUrl,
...
export async function POST(request) {
  try {
    ...
    const fullModelId = `oryphemrouter/${model}`;

    // Remove all old oryphemrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("oryphemrouter/"))
    ...
    // Add fresh oryphemrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`oryphemrouter/${m}`] = {};
    });

    // Remove old oryphemrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("oryphemrouter/")) {
    ...
    // Update models.providers.oryphemrouter with all models
    settings.models.providers["oryphemrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `oryphemrouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove oryphemrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("oryphemrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses oryphemrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("oryphemrouter/")) {
