...
export default function CopilotToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    if (status?.config && Array.isArray(status.config) && selectedModels.length === 0) {
      const entry = status.config.find((e) => e.name === "oryphemrouter");
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const res = await fetch("/api/cli-tools/all-statuses?tool=copilot");
      const data = await res.json();
      if (!res.ok) {
        ...
        setStatus(data?.copilot || { checkFailed: true, error: "No status for copilot" });
      }
    } catch (error) {
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div
          role="button"
          tabIndex={0}
          aria-label="Toggle Copilot tool"
