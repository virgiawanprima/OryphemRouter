...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("oryphemrouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("oryphemrouter/", ""));
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_oryphemrouter" : selectedApiKey);
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasoryphemrouter) return "not_configured";
    const url = status.config?.provider?.["oryphemrouter"]?.options?.baseURL || "";
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `oryphemrouter/${activeModelToShow}`,
        agent: {
          explorer: {
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div
          role="button"
          tabIndex={0}
          aria-label="Toggle OpenCode tool"
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["oryphemrouter"]?.options?.baseURL && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasoryphemrouter} loading={restoring}>
