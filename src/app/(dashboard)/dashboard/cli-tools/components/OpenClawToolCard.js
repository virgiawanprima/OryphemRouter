...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["oryphemrouter"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["oryphemrouter"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setSelectedModel(primaryModel.replace("oryphemrouter/", ""));
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse = selectedApiKey?.trim()
        ...
        || (!cloudEnabled ? "sk_oryphemrouter" : null);
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_oryphemrouter" : "<API_KEY_FROM_DASHBOARD>");

    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `oryphemrouter/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
    ...
    <Card padding="xs" className="overflow-hidden">
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          {!checkingOpenclaw && openclawStatus?.checkFailed && (
          ...
          {!checkingOpenclaw && openclawStatus && !openclawStatus.checkFailed && !openclawStatus.installed && (
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if oryphemrouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["oryphemrouter"]?.baseUrl && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <Input size="small" className="w-full min-w-0 pl-2 pr-7 text-xs" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="provider/model-id" />
                ...
                {(openclawStatus.agents || []).filter(a => a.agentDir).map((agent) => (
                  <div key={agent.id} className="flex items-center gap-2 pl-4">
                    ...
                    <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                    <div className="relative w-full min-w-0">
                      <Input value={agentModels[agent.id] || ""} onChange={(e) => setAgentModels(prev => ({ ...prev, [agent.id]: e.target.value }))} placeholder={`default (${selectedModel || "provider/model-id"})`} className="w-full" />
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasoryphemrouter} loading={restoring}>
