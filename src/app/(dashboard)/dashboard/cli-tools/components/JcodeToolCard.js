...
export default function JcodeToolCard({
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["oryphemrouter"];
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.oryphemrouter]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_9ROUTER_API_KEY"
env_file = "provider-oryphemrouter.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.oryphemrouter.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-oryphemrouter.env",
        content: envContent,
      },
    ];
  };
    ...
    <Card padding="xs" className="overflow-hidden">
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          {!checkingJcode && jcodeStatus?.checkFailed && (
          ...
          {!checkingJcode && jcodeStatus && !jcodeStatus.checkFailed && !jcodeStatus.installed && (
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if oryphemrouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["oryphemrouter"]?.base_url && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <Input size="small" className="w-full min-w-0 pl-2 pr-7 text-xs" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="cc/claude-opus-4-7" />
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile oryphemrouter</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile oryphemrouter --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
