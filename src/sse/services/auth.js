...
// Circuit breaker: track consecutive errors per connection
...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  try {
    ...
    if (availableConnections.length === 0) {
      ...
      const earliest = expiries.sort((a, b) => new Date(a).getTime() - new Date(b).getTime())[0] || null;
      if (earliest) {
...
export async function markAccountUnavailable(connectionId, status, errorText, provider = null, model = null, resetsAtMs = null) {
  ...
  if (shouldFallback) {
    ...
    const currentErrorCount = (conn?.circuitBreakerErrorCount || 0) + 1;

    if (currentErrorCount >= CIRCUIT_BREAKER_THRESHOLD) {
      // Circuit open: force 5-minute cooldown regardless of normal backoff
      const circuitCooldownMs = CIRCUIT_BREAKER_COOLDOWN_MS;
      cooldownMs = Math.max(cooldownMs || 0, circuitCooldownMs);
      log.warn("CIRCUIT", `Circuit breaker opened: ${currentErrorCount} consecutive errors, cooling down ${Math.round(circuitCooldownMs / 1000)}s`);
    }

    await updateProviderConnection(connectionId, {
      circuitBreakerErrorCount: currentErrorCount,
      circuitBreakerLastErrorAt: now,
    });
  }
  ...
  if (provider && status && reason) {
    log.warn("AUTH", `${provider} [${status}]: ${reason}`);
  }
...
export async function clearAccountError(connectionId, currentConnection, model = null) {
  ...
  if (remainingActiveLocks.length === 0) {
    Object.assign(clearObj, {
      ...
      lastErrorAt: null,
      backoffLevel: 0,
      circuitBreakerErrorCount: 0,
      circuitBreakerLastErrorAt: null
    });
  }
