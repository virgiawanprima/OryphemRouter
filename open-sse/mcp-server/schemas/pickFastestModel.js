import { z } from "zod";
const pickFastestModelInput = z.object({
  comboId: z.string().optional().describe(
    "Optional combo id or name to scope the ranking to. Omit to rank across all enabled combos."
  ),
  includeUnhealthy: z.boolean().optional().describe(
  ...
  }).partial().optional().describe("Optional speed-ranking weight overrides merged onto the defaults."),
  applyToCombo: z.boolean().optional().describe("When true + comboId present, switches the combo to auto/latency routing."),
  limit: z.number().int().min(1).max(50).optional().describe("Ranked result limit.")
});
const pickFastestModelOutput = z.object({
  fastest: z.object({
    provider: z.string(),
    model: z.string(),
    score: z.number(),
...
const pickFastestModelTool = {
  name: "omniroute_pick_fastest_model",
  description: "Picks the fastest reliable provider-model pair from live telemetry and can apply latency routing to a combo.",
