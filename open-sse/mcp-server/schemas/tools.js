...
import { pickFastestModelTool } from "./pickFastestModel.js";
import { getActiveSearchProviders } from "./providerEnums.js";
import { CCR_MCP_TOOLS } from "./ccrTools.js";
...
const createComboInput = z.object({
  ...
  ).min(1).describe("Ordered model chain; order defines priority")
});
const createComboOutput = z.object({
  success: z.boolean(),
  combo: z.object({
    id: z.string(),
    name: z.string(),
...
const createComboTool = {
  ...
  description: "Registers a new combo (model chain) with a name, ordered model list, and optional routing strategy. Full validation (name collisions, nested-combo DAG, composite tiers) is enforced by the combos API.",
...
function buildWebSearchInputSchema(blockedProviders = []) {
  return z.object({
    ...
    provider: z.enum(getActiveSearchProviders(blockedProviders)).optional().describe("Specific search provider to use")
  });
}
const webSearchInput = buildWebSearchInputSchema();
...
const xSearchInput = z.object({
  query: z.string().min(1, "Query is required").max(500, "Query must be 500 characters or fewer").describe("X search query (keywords, topic, or @handle)"),
  max_results: z.number().int().min(1).max(20).default(5).describe("Maximum number of X results to return")
});
const xSearchTool = {
  name: "omniroute_x_search",
  description: "Search X (Twitter) through OmniRoute using SuperGrok / xAI server-side x_search. Requires a connected xai-oauth (SuperGrok) or xAI API key. This is Grok X Search, not web search and not the X Developer Platform MCP.",
...
const webFetchInput = z.object({
  ...
  provider: z.enum(["firecrawl", "jina-reader", "tavily-search", "tinyfish", "context7"]).optional().describe(
    "Specific fetch provider to use (default: first available). context7 expects a library reference URL (context7.com/<owner>/<repo>) and is explicit-only."
  ),
...
const webFetchTool = {
  ...
  description: "Fetches and extracts content from a URL using OmniRoute's web fetch gateway. Supports multiple providers (Firecrawl, Jina Reader, Tavily, TinyFish, Context7 library docs) with automatic failover. Returns the page content as markdown, HTML, links, or screenshot, along with metadata.",
...
const compressionConfigureInput = z.object({
  ...
  strategy: z.enum([
    ...
    "stacked",
    "omniglyph"
  ]).optional().describe("Compression mode"),
  autoTriggerMode: z.enum([
    "off",
    "lite",
    "standard",
    "aggressive",
...
const compressionConfigureTool = {
  ...
  description: "Configure compression settings at runtime. Supports enabling/disabling compression, changing strategy (off/lite/standard/aggressive/ultra/rtk/codex-responses/stacked), adjusting maxTokens threshold, targetRatio, auto-trigger mode, system prompt preservation, and MCP description compression.",
...
import { toolSearchInput, toolSearchOutput, toolSearchTool as toolSearchTool2 } from "./toolSearch.js";
const MCP_TOOLS = [
  toolSearchTool,
  getHealthTool,
  listCombosTool,
