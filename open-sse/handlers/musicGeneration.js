import { getMusicProvider, parseMusicModel } from "../utils/omni/musicRegistry.js";
import { kieExecutor } from "../utils/omni/kie.js";
import { vertexGenerateMusic } from "../utils/omni/vertexMedia.js";
import {
  submitComfyWorkflow,
  pollComfyResult,
  fetchComfyOutput,
...
import { handleFalMusicGeneration } from "./mediaGeneration/fal.js";
import { handleMinimaxMusicGeneration } from "./mediaGeneration/minimaxMusic.js";
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate) && candidate.length > 0) {
      return candidate.map(
        (track) => isJsonObject(track) ? track : typeof track === "string" ? { audioUrl: track } : null
...
async function handleMusicGeneration({ body, credentials, log }) {
  ...
  if (providerConfig.format === "vertex-lyria") {
    try {
      const { base64, format } = await vertexGenerateMusic(credentials, {
        ...
        sampleCount: typeof body.sample_count === "number" ? body.sample_count : void 0,
        seed: typeof body.seed === "number" ? body.seed : void 0
      });
      return {
        success: true,
        data: { created: Math.floor(Date.now() / 1e3), data: [{ b64_json: base64, format }] }
      };
    } catch (err) {
...
async function handleComfyUIMusicGeneration({ model, provider, providerConfig, body, log }) {
  ...
  try {
    ...
    return {
      success: true,
      data: { created: Math.floor(Date.now() / 1e3), data: audioFiles }
    };
  } catch (err) {
...
async function handleKieMusicGeneration({
  ...
  try {
    ...
    if (!taskId) {
      const errorMessage2 = createData?.msg || createData?.message || createData?.error || "KIE music generation did not return taskId";
      if (log) {
    ...
    if (state === "success") {
      ...
      const audioFiles = tracks.map(
        (track) => typeof track.audioUrl === "string" ? track.audioUrl : typeof track.audio_url === "string" ? track.audio_url : typeof track.url === "string" ? track.url : null
      ).filter((url2) => typeof url2 === "string" && url2.length > 0).map((url2) => ({ url: url2, format: "mp3" }));
      saveCallLog({
        method: "POST",
        path: "/v1/music/generations",
        status: 200,
