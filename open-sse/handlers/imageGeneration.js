...
import { getImageProvider, parseImageModel } from "../utils/omni/imageRegistry.js";
import { applyAntigravityClientProfileHeaders } from "../utils/omni/antigravityProfileHeaders.js";
import { getAntigravityEnvelopeUserAgent } from "../utils/omni/antigravityIdentity.js";
import { kieExecutor } from "../utils/omni/kie.js";
import { mapImageSize } from "../utils/omni/sizeMapper.js";
import { getCodexClientVersion, getCodexUserAgent } from "../utils/omni/codexClientVersion.js";
import { ChatGptWebExecutor } from "../executors/chatgpt-web.js";
import { getChatGptImage, findChatGptImageBySha256 } from "../services/chatgptImageCache.js";
...
import { sanitizeUpstreamDetails } from "../utils/omni/errorExtras.js";
import { handleSDWebUIImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleHyperbolicImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleHuggingFaceImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleComfyUIImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleImagen3ImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleIdeogramImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleHaiperImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleLeonardoImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleMagnificImageGeneration } from "../utils/omni/imageGenProviders.js";
import {
  handleChatGptWebImageGeneration,
  extractMarkdownImageUrls,
  CHATGPT_WEB_IMAGE_ID_RE
} from "../utils/omni/imageGenProviders.js";
import { handleGeminiWebImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleNvidiaNimImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleSegmindImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleDesignerWebImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleCursorAgentImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleMinimaxImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleAdobeFireflyImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleAlibabaImageGeneration } from "../utils/omni/imageGenProviders.js";
import { handleAiHordeImageGeneration } from "../utils/omni/imageGenProviders.js";
import {
  applyPollinationsAnonymousFallback,
  reportPollinationsAnonOutcome
} from "../utils/omni/imageGenProviders.js";
const KIE_MARKET_UPSTREAM_MODEL_IDS = /* @__PURE__ */ new Map([
...
function resolveKieMarketUpstreamModelId(publicModelId) {
  return KIE_MARKET_UPSTREAM_MODEL_IDS.get(publicModelId) ?? publicModelId;
}
const OPENAI_IMAGE_TO_IMAGE_MODELS = /* @__PURE__ */ new Set([
...
function resolveImageBaseUrl(credentials, fallback, endpoint = "generations") {
...
function isCodexChatGptModelAccessError(status, errorText, model) {
  ...
  if (typeof parsed === "string") {
    ...
    if (typeof obj.detail === "string") detail = obj.detail;
      ...
      const nested = obj.error.message;
...
const BFL_EDIT_MODELS = /* @__PURE__ */ new Set([
  "flux-2-max",
  "flux-2-pro",
  "flux-2-flex",
  "flux-kontext-pro",
...
const BFL_FAILURE_STATUSES = /* @__PURE__ */ new Set(["Error", "Failed", "Content Moderated", "Request Moderated"]);
...
const STABILITY_CONTROL_MODELS = /* @__PURE__ */ new Set(["sketch", "structure", "style", "style-transfer"]);
...
async function handleImageGeneration({
  ...
  if (providerConfig.format === "agnes-image" && (typeof body.size !== "string" || body.size.trim().length === 0)) {
    return {
      success: false,
      status: 400,
  ...
  if (providerConfig.format === "alibaba-image" || providerConfig.format === "qwen-cloud-image" || providerConfig.format === "qwen-token-plan-image" || providerConfig.format === "bailian-coding-plan-image") {
...
async function handleKieImageGeneration({
  ...
  const timeoutMs = normalizePositiveNumber(body.timeout_ms, 3e5);
  const pollIntervalMs = normalizePositiveNumber(body.poll_interval_ms, 2500);
  const prompt = typeof body.prompt === "string" ? body.prompt : String(body.prompt ?? "");
  const size = typeof body.size === "string" ? body.size : void 0;
  if (!token) {
  ...
  try {
    ...
    if (!taskId) {
      const errorMessage2 = createData?.msg || createData?.message || createData?.error || "KIE image generation did not return taskId";
      if (log) {
    ...
    if (state === "success") {
      ...
      const images = urls.map((url) => ({ url, revised_prompt: prompt }));
...
function buildAgnesImageRequestBody(model, body) {
  ...
  if (body.size !== void 0) upstreamBody.size = body.size;
  if (body.ratio !== void 0) {
    upstreamBody.ratio = body.ratio;
  } else if (body.aspect_ratio !== void 0) {
    upstreamBody.ratio = body.aspect_ratio;
  }
  if (body.return_base64 !== void 0) upstreamBody.return_base64 = body.return_base64;
  const explicitExtraBody = body.extra_body && typeof body.extra_body === "object" && !Array.isArray(body.extra_body) ? body.extra_body : {};
  const extraBody = { ...explicitExtraBody };
  const { imageUrls } = extractImageInputs(body);
  if (imageUrls.length > 0) extraBody.image = imageUrls;
  if (body.response_format !== void 0) extraBody.response_format = body.response_format;
...
async function handleOpenAIImageGeneration({
  ...
  const logRequestBody = {
    ...
    size: body.size || "default",
    n: body.n || 1,
    quality: body.quality || void 0
  };
  const upstreamBody = providerConfig.format === "agnes-image" ? buildAgnesImageRequestBody(model, body) : {
    model,
    prompt: body.prompt
  };
  if (providerConfig.format !== "agnes-image") {
    if (body.n !== void 0) upstreamBody.n = body.n;
    if (body.size !== void 0) upstreamBody.size = body.size;
    if (body.quality !== void 0) upstreamBody.quality = body.quality;
    if (body.response_format !== void 0) upstreamBody.response_format = body.response_format;
    if (body.style !== void 0) upstreamBody.style = body.style;
    const { imageUrl } = extractImageInputs(body);
  ...
  let pollinationsAnonSession = null;
...
async function handleOpenAIImageEdit({
  ...
  partBuffers.push(
    Buffer.from(
      `--${boundary}${CRLF}Content-Disposition: form-data; name="image"; filename="image.png"${CRLF}Content-Type: ${imageMime || "image/png"}${CRLF}${CRLF}`
    )
  );
...
async function handleOpenRouterImageEdit({
  model,
  provider,
  baseUrl,
  credentials,
...
async function handleFalAIImageGeneration({
  ...
  if (body.n !== void 0) upstreamBody.num_images = Number(body.n) || 1;
  if (body.negative_prompt) upstreamBody.negative_prompt = body.negative_prompt;
  if (body.seed !== void 0) upstreamBody.seed = body.seed;
...
async function handleBlackForestLabsImageGeneration({
  ...
  try {
    ...
    if (body.seed !== void 0) upstreamBody.seed = body.seed;
    if (body.n !== void 0 && model.includes("ultra"))
      upstreamBody.num_images = Number(body.n) || 1;
    if (body.quality === "hd" && model.includes("ultra")) upstreamBody.raw = true;
    if (body.left !== void 0) upstreamBody.left = body.left;
    if (body.right !== void 0) upstreamBody.right = body.right;
    if (body.top !== void 0) upstreamBody.top = body.top;
    if (body.bottom !== void 0) upstreamBody.bottom = body.bottom;
    if (body.steps !== void 0) upstreamBody.steps = body.steps;
    if (body.guidance !== void 0) upstreamBody.guidance = body.guidance;
    if (body.grow_mask !== void 0) upstreamBody.grow_mask = body.grow_mask;
    if (body.safety_tolerance !== void 0) upstreamBody.safety_tolerance = body.safety_tolerance;
    if (log) {
      const promptPreview = String(body.prompt ?? "").slice(0, 60);
...
async function handleTopazImageGeneration({
  ...
  try {
    ...
    const wantsBase64 = body.response_format === "b64_json";
    const images = [
      wantsBase64 ? { b64_json: base64, revised_prompt: body.prompt } : { url: `data:${contentType};base64,${base64}`, revised_prompt: body.prompt }
    ];
    return saveImageSuccessResult({
      provider,
      model,
...
function extractImageGenerationCalls(sseText) {
  const results = [];
...
async function handleCodexImageGeneration({
  ...
  const workspaceId = credentials?.providerSpecificData && typeof credentials.providerSpecificData === "object" && !Array.isArray(credentials.providerSpecificData) ? credentials.providerSpecificData.workspaceId : void 0;
  const toolConfig = { type: "image_generation", output_format: "png" };
  ...
  const upstreamBody = {
    model,
    instructions: referenceImages.length > 0 ? `You must call the image_generation tool exactly once to edit the supplied ${referenceImages.length === 1 ? "reference image" : "reference images"}. Treat all supplied images as references for the user's requested composition or style. Do not add narration.` : "You must call the image_generation tool exactly once to fulfill the user's request. Do not add narration.",
    input: [
      {
        role: "user",
  ...
  const requestBodyForLog = referenceImages.length > 0 ? {
    model,
  ...
  if (log) {
    const promptSummary = referenceImages.length > 0 ? `${prompt.length} chars` : `"${prompt.slice(0, 60)}..."`;
...
async function handleCodexImageEdit({
  model,
  provider,
  providerConfig,
  body,
...
async function handleNanoBananaImageGeneration({
  ...
  try {
    ...
    if (hasSyncPayload) {
      ...
      return {
        success: true,
        data: { created: Math.floor(Date.now() / 1e3), data: syncResult.data }
      };
    }
    ...
    const timeoutMs = normalizePositiveNumber(
      body.timeout_ms,
      normalizePositiveNumber(process.env.NANOBANANA_POLL_TIMEOUT_MS, 12e4)
    );
...
export {
  ...
  resolveImageBaseUrl,
  resolveKieMarketUpstreamModelId,
