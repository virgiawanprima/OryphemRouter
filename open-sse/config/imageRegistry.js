import { LMARENA_DIRECT_IMAGE_MODELS } from "../providers/registry/lmarena/directModels.js";
import { SEGMIND_IMAGE_PROVIDER } from "../providers/registry/segmind/imageModels.js";
import { KIE_IMAGE_MODELS } from "../providers/registry/kie/imageModels.js";
import { MAGNIFIC_IMAGE_PROVIDER } from "../providers/registry/magnific/index.js";
import { STABILITY_AI_IMAGE_MODELS } from "../providers/registry/stability-ai/imageModels.js";
import { CHEAPERINFERENCE_IMAGE_PROVIDER } from "../providers/registry/cheaperinference/imageModels.js";
import {
  ADOBE_FIREFLY_IMAGE_ROUTING_ALIASES,
  toRegistryImageModels
} from "../services/adobeFireflyModels.js";
import { AI_HORDE_IMAGE_PROVIDER } from "../providers/registry/aihorde/imageModels.js";
...
const IMAGE_PROVIDERS = {
  ...
  "bailian-coding-plan": {
    ...
    alias: "bcp",
    baseUrl: "https://coding-intl.dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation",
    authType: "apikey",
    authHeader: "bearer",
...
function getImageProvider(providerId) {
...
function parseImageModel(modelStr) {
  ...
  for (const [providerId, config] of Object.entries(IMAGE_PROVIDERS)) {
    if (modelStr.startsWith(providerId + "/")) {
      ...
      const aliased = resolveImageModelAlias(`${providerId}/${model}`) || resolveSameProviderBareAlias(providerId, model);
...
function imageAliasCatalogEntry(alias, target) {
  ...
  return {
    ...
    description: target.description || modelConfig?.description || void 0
  };
}
function getAllImageModels() {
  const providerModels = Object.entries(IMAGE_PROVIDERS).flatMap(
    ([providerId, config]) => imageProviderCatalogEntries(providerId, config)
  );
...
function getImageModelAliases() {
  return IMAGE_MODEL_ALIASES;
}
function isRegisteredImageModel(providerId, modelId) {
  return Boolean(findImageModelConfig(providerId, modelId));
}
function getImageModelEntry(modelStr) {
  if (!modelStr) return null;
  const alias = IMAGE_MODEL_ALIASES[modelStr];
  if (alias) {
    const modelConfig2 = findImageModelConfig(alias.provider, alias.model);
    return {
      provider: alias.provider,
      model: alias.model,
      inputModalities: alias.inputModalities || modelConfig2?.inputModalities || ["text"],
      imageRequired: resolveAliasImageRequired(alias, modelConfig2),
      description: alias.description || modelConfig2?.description || void 0
    };
  }
  ...
  return {
    ...
    description: modelConfig.description || void 0
  };
}
function modalitiesRequireImageInput(inputModalities) {
