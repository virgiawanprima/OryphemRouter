import { parseModelFromRegistry } from "./registryUtils.js";
import { RUNWAYML_SUPPORTED_VIDEO_MODELS } from "./runway.js";
import { SEGMIND_VIDEO_MODELS } from "../providers/registry/segmind/videoModels.js";
import { toRegistryVideoModels } from "../services/adobeFireflyModels.js";
const VIDEO_PROVIDERS = {
  ...
  googleflow: {
    ...
    unsupported: true,
    unsupportedReason: "Google Flow video generation requires a browser-session transport (Flow/Cloud Code session with reCAPTCHA) and is not supported over the stored OAuth bearer. Generate video via labs.google/flow directly for now."
  },
  kie: {
    id: "kie",
...
function getVideoProvider(providerId) {
  return VIDEO_PROVIDERS[providerId] || null;
}
function parseVideoModel(modelStr) {
  return parseModelFromRegistry(modelStr, VIDEO_PROVIDERS);
}
function getAllVideoModels() {
  return Object.entries(VIDEO_PROVIDERS).filter(([, config]) => !config.unsupported).flatMap(
    ([providerId, config]) => [providerId, config.alias].filter((prefix) => Boolean(prefix)).flatMap(
      (prefix) => config.models.map((model) => ({
        id: `${prefix}/${model.id}`,
        name: model.name,
        provider: providerId,
