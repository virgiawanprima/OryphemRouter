...
// AgentService can carry a full tool conversation (schemas + tool_calls/results);
// only modality-bearing content (images etc.) or a missing message list blocks it.
export function isAgentCapableRequest(body) {
  ...
  return body.messages.every((message) => {
    ...
    return message.content.every((part) => !part || part?.type === "text");
  });
}
...
export function buildAgentRunFrame(messages, model, tools) {
  ...
  const mcpTools = Array.isArray(tools) && tools.length
    ? encodeMcpTools(tools.map((t) => t?.function || t))
    : null;
...
function createRequestContextResponse() {
  // AgentService asks every run for client context. oryphemrouter has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; oryphemrouter is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that oryphemrouter cannot service. Fail the
