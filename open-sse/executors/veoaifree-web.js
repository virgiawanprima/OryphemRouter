...
const ARTIFACT_READY_RETRY_INTERVAL_MS = 5e3;
const ARTIFACT_READY_MAX_ATTEMPTS = 12;
const RETRYABLE_ARTIFACT_STATUSES = /* @__PURE__ */ new Set([202, 404, 409, 425]);
...
function detectIntent(model, prompt) {
...
async function handleImage(context, nonce, prompt, aspectRatio) {
  ...
  const images = parts.map(
    (p) => p.startsWith("http") ? { url: p, type: "image" } : { b64_json: p, type: "image" }
  );
  ...
  try {
    ...
    if (error instanceof OutboundUrlGuardError) {
      throw new VeoArtifactError(
        error.code === "OUTBOUND_URL_INVALID" ? "VIDEO_ARTIFACT_URL_INVALID" : "VIDEO_ARTIFACT_URL_BLOCKED",
        error.code === "OUTBOUND_URL_INVALID" ? "Video artifact URL is invalid" : "Video artifact URL is blocked",
        502
      );
    }
...
class VeoAIFreeWebExecutor extends BaseExecutor {
  constructor() {
  ...
  async execute(input) {
    ...
    if (intent === "tts") {
      ...
      const resp2 = await handleTTS(prompt, voiceMatch?.[1], langMatch?.[1], input.signal);
      return { response: resp2, url: TTS_URL, headers: {}, transformedBody: { intent, model } };
    }
    const context = buildSessionContext(input.signal || void 0);
    let nonce;
    try {
