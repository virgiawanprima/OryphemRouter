...
function resolveKimiProtocol(credentials, body) {
  ...
  if (record?.system !== void 0 || record?.output_config !== void 0 || record?.context_management !== void 0) {
    return "claude";
  }
  return "openai";
}
function getThinkingPolicy(credentials) {
...
function resolveRequestedEffort(body) {
...
function constrainEffort(effort, policy) {
  ...
  return policy.defaultThinkingEffort && supported.includes(policy.defaultThinkingEffort) ? policy.defaultThinkingEffort : "on";
}
function applyThinkingPolicyDefaults(requestedEffort, policy) {
...
function buildOpenAIThinking(effort, currentThinking, policy) {
  const constrained = constrainEffort(effort, policy);
  const previousKeep = asRecord(currentThinking)?.keep;
  if (constrained === "off") {
    return { type: "disabled", ...previousKeep !== void 0 ? { keep: previousKeep } : {} };
  }
  return {
    type: "enabled",
...
function normalizeExistingOpenAIThinking(value) {
  ...
  return {
    ...thinking,
    ...type ? { type } : {},
    ...type && type !== "disabled" && thinking.keep === void 0 ? { keep: "all" } : {}
  };
}
function applyOpenAIThinking(body, policy) {
...
function backfillKimiReasoningContent(body) {
  const thinking = asRecord(body.thinking);
  ...
  const messages = body.messages.map((message) => {
    ...
    if (!record || record.role !== "assistant" || !hasAssistantToolCalls(record) || Object.hasOwn(record, "reasoning_content")) {
      return message;
    }
    changed = true;
...
function normalizeOpenAIRequest(body, stream, policy) {
  let next = { ...body };
  if (next.max_completion_tokens === void 0 && next.max_tokens !== void 0) {
...
function budgetToEffort(value) {
  if (typeof value !== "number" || !Number.isFinite(value)) return null;
  if (value <= 1024) return "low";
  if (value <= 10240) return "medium";
  return "high";
}
function removeClearThinkingEdit(body) {
  ...
  if (edits.length > 0) {
    ...
    const betas = Array.isArray(body.betas) ? body.betas.filter((beta) => beta !== KIMI_CONTEXT_MANAGEMENT_BETA) : null;
    if (betas?.length) body.betas = betas;
    else if (betas) delete body.betas;
  }
}
function addClearThinkingKeep(body) {
  const contextManagement = asRecord(body.context_management) || {};
...
function backfillKimiAnthropicThinking(body) {
...
function firstNormalizedEffort(...values) {
...
function resolveAnthropicEffort(body, existingThinking, outputConfig, policy) {
  const reasoning = asRecord(body.reasoning);
...
function applyAnthropicEffort(body, effort, outputConfig, policy) {
...
function normalizeExistingAnthropicThinking(body, existingThinking) {
...
function normalizeAnthropicRequest(body, policy) {
  const next = { ...body };
...
class KimiExecutor extends DefaultExecutor {
  ...
  buildUrl(model, stream, urlIndex = 0, credentials = null) {
    ...
    void stream;
    void urlIndex;
    return resolveKimiProtocol(credentials) === "claude" ? KIMI_CODING_ANTHROPIC_URL : KIMI_CODING_OPENAI_URL;
  }
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    const normalized = resolveKimiProtocol(credentials, withNames) === "claude" ? normalizeAnthropicRequest(withNames, policy) : normalizeOpenAIRequest(withNames, stream, policy);
