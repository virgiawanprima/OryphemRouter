...
function extractAssistantContent(message) {
  ...
  const parts = collectRendererTexts(contentRenderer, /* @__PURE__ */ new Set());
  return parts.join("\n\n").trim();
}
function extractAssistantReasoning(message) {
  const parts = collectReasoningTexts(message, /* @__PURE__ */ new Set());
  return parts.join("\n\n").trim();
}
...
export {
  META_AI_NESTED_RENDERER_KEYS,
  META_AI_REASONING_KEYS,
  classifyMetaAiError,
  collectReasoningTexts,
  collectRendererTexts,
