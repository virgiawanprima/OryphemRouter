...
class FreebuffExecutor extends BaseExecutor {
  constructor() {
  ...
  async execute(input) {
    ...
    const incomingMessages = Array.isArray(payload.messages) ? payload.messages.filter(
      (message) => !!message && typeof message === "object" && !Array.isArray(message)
    ) : [];
    const firstMessage = incomingMessages[0];
    const hasBuffyPrompt = incomingMessages.length > 0 && firstMessage?.role === "system" && typeof firstMessage.content === "string" && firstMessage.content.trim().startsWith("You are Buffy");
    if (!hasBuffyPrompt) {
    ...
    const existingMetadata = payload.codebuff_metadata && typeof payload.codebuff_metadata === "object" && !Array.isArray(payload.codebuff_metadata) ? payload.codebuff_metadata : {};
    const upstreamBody = {
      ...payload,
