...
async function solveDuckDuckGoChallenge(challenge, userAgent, options = {}) {
  ...
  vm.runInContext(stubs, context, { timeout: 5e3 });
...
function makeDuckDuckGoFeSignals() {
  ...
  const payload = {
    start,
    events,
    end: Math.max(delta + 20 + Math.floor(Math.random() * 71), 3e3)
  };
