...
function resolveChatGptSystemHints(model) {
  return [...MODEL_SYSTEM_HINTS[model] ?? []];
}
function resolveChatGptModel(model, _body, _providerSpecificData) {
  const slug = MODEL_MAP[model] ?? model;
  const effort = MODEL_FORCED_EFFORT[model] ?? null;
  const isPro = model === "gpt-5.6-sol-pro" || model === "gpt-5.5-pro" || model === "gpt-5.5-pro-extended" || slug === "gpt-5-6-pro" || slug === "gpt-5-5-pro";
