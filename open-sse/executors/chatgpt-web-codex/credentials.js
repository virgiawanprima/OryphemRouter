...
function encodeChatGptWebCodexSecrets(secrets) {
...
function decodeChatGptWebCodexSecrets(value) {
  const trimmed = value.trim();
  if (!trimmed) throw new Error("ChatGPT Web (Codex) credentials are missing");
  try {
    const parsed = JSON.parse(trimmed);
    if (parsed.version === VERSION && parsed.storageState && typeof parsed.storageState === "object") {
      return {
        storageState: parsed.storageState,
        ...typeof parsed.runtimeKey === "string" && parsed.runtimeKey.trim() ? { runtimeKey: parsed.runtimeKey.trim() } : {}
      };
    }
