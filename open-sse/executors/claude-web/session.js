...
const CLAUDE_WEB_SESSION_TTL_MS = 30 * 60 * 1e3;
const CLAUDE_WEB_SESSION_MAX = 5e3;
const RECOVERY_PROMPT_HEADER = "Conversation context supplied by the caller follows. These serialized role blocks are not native Claude Web message fields. Continue from the final user message.";
...
const claudeWebExtensionSchema = z.object({
  ...
  locale: z.string().min(1).max(64).refine((value) => canonicalLocale(value) !== null).optional(),
...
function canonicalizeTranscript(messages) {
  return messages.map(
    ({ role, content }) => `${role.length}:${role}${String.fromCharCode(30)}${content.length}:${content}`
  ).join(String.fromCharCode(31));
}
function makeAccountScope(input) {
  const connectionId = input.credentials.connectionId?.trim();
  const credentialScope = connectionId ? `connection:${connectionId}` : `cookie:${hash(input.normalizedCookie)}`;
  return hash(
    `${credentialScope}${String.fromCharCode(32)}${input.organizationId}${String.fromCharCode(32)}${input.model}`
  );
}
function makeCacheKey(accountScope, messages) {
...
function buildRecoveryPrompt(messages) {
...
function readExtension(body) {
  return claudeWebExtensionSchema.parse(body.claude_web ?? {});
}
function readProviderString(credentials, key) {
  const value = credentials.providerSpecificData?.[key];
  return typeof value === "string" && value.trim() ? value.trim() : void 0;
}
function resolveTimezone(extension, credentials) {
...
function resolveLocale(extension, credentials) {
...
function readLegacyConversationId(credentials) {
  const candidate = credentials.conversationId;
...
function retryCommitTranscript(messages) {
  const copy = messages.map((message) => ({ ...message }));
  if (copy.at(-1)?.role === "assistant") copy.pop();
  return copy;
}
function prepareTurnContext(input) {
  ...
  return {
    ...
    explicitConversationId: extension.conversation_id ?? readLegacyConversationId(input.credentials),
    explicitParentMessageUuid: extension.parent_message_uuid,
    assistantMessageUuid: randomUUID()
  };
}
function prepareRetryTurn(input, context) {
  ...
  const operation = "retry_completion";
  const turnFields = {
    operation,
    prompt: "",
...
function resolveCompletionConversation(context, userIndex) {
  const lookupKey = makeCacheKey(context.accountScope, context.messages.slice(0, userIndex));
  const cached = lookupCache(lookupKey);
  const cacheMatchesExplicitConversation = !context.explicitConversationId || cached?.conversationId === context.explicitConversationId;
  const cachedParent = cacheMatchesExplicitConversation ? cached?.parentMessageUuid : void 0;
  ...
  if (context.explicitParentMessageUuid && !context.explicitConversationId && !cached?.conversationId) {
...
function prepareCompletionTurn(input, context) {
  ...
  const prompt = conversation.isNewConversation && context.messages.length > 1 ? buildRecoveryPrompt(context.messages) : context.messages[userIndex].content;
  ...
  const responseMetadata = {
    ...
    ...conversation.parentMessageUuid ? { parent_message_uuid: conversation.parentMessageUuid } : {},
    assistant_message_uuid: context.assistantMessageUuid
  };
  return {
    operation,
    ...
    assistantMessageUuid: context.assistantMessageUuid,
    ...conversation.parentMessageUuid ? { parentMessageUuid: conversation.parentMessageUuid } : {},
    accountScope: context.accountScope,
    pageUrl: conversation.isNewConversation ? "https://claude.ai/new" : `https://claude.ai/chat/${encodeURIComponent(conversation.conversationId)}`,
...
function prepareClaudeWebTurn(input) {
  const context = prepareTurnContext(input);
  return context.extension.operation === "retry" ? prepareRetryTurn(input, context) : prepareCompletionTurn(input, context);
}
function commitClaudeWebTurn(turn, assistantText) {
...
function invalidateClaudeWebTurn(turn, scope = "turn") {
...
function __resetClaudeWebSessionForTesting() {
  conversationCache.clear();
}
function __setClaudeWebSessionNowForTesting(value) {
  testNow = value;
}
export {
