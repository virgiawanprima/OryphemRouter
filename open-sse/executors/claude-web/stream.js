...
const KNOWN_METADATA_EVENTS = /* @__PURE__ */ new Set([
  "ping",
  "completion",
  "message_limit",
...
async function* decodeSseData(source, control) {
...
function projectMetadataEvent(eventType, event) {
  const projected = { type: eventType };
...
function thinkingSummaryText(delta) {
  ...
  if (delta.summary && typeof delta.summary === "object" && !Array.isArray(delta.summary)) {
    return deltaText(delta.summary, ["summary", "text", "thinking"]);
  }
...
function dispatchProtocolEvent(eventType, event, state) {
  switch (eventType) {
    case "message_start":
...
async function* parseClaudeWebEvents(source, control) {
  const state = {
...
function makeChunk(id, created, options, delta, finishReason, event) {
  return {
    id,
...
function responseHeaders(contentType, metadata) {
...
function notifyComplete(options, result) {
  try {
...
async function createBufferedResponse(source, options) {
  ...
  const control = { reader: null, cancelled: false };
  try {
...
function encodeStreamEvent(state, value) {
  return state.encoder.encode(`data: ${JSON.stringify(value)}

`);
}
function failStreamOnce(state, options) {
  if (state.outcome !== "pending") return;
  state.outcome = "failed";
  notifyFailure(options);
}
function closeStreamIfDrained(state, controller) {
  if (!state.closed && state.terminal && state.pendingChunks.length === 0) {
    state.closed = true;
    controller.close();
  }
}
function flushStreamChunk(state, controller) {
...
async function queueSemanticEvent(state, event, options) {
...
function queueStreamFailure(state, options) {
  options.log?.error?.("CLAUDE-WEB-STREAM", "Claude Web stream protocol validation failed");
...
async function pullStreamingChunk(state, options, controller) {
...
async function cancelStreaming(state, options, reason) {
...
function createStreamingResponse(source, options) {
  const control = { reader: null, cancelled: false };
  const state = {
...
async function createClaudeWebResponse(source, options) {
  return options.stream ? createStreamingResponse(source, options) : createBufferedResponse(source, options);
}
export {
  createClaudeWebResponse
};
