import { ADOBE_FIREFLY_DISCOVERY_SNAPSHOT } from "./adobeFireflyModelSnapshot.js";
...
function mergeAdobeObjectSchema(schema) {
...
function enumStrings(schema) {
  return [
    ...new Set(
      schemaBranches(schema).flatMap((branch) => Array.isArray(branch.enum) ? branch.enum : []).filter((value) => typeof value === "string")
    )
  ];
}
function integerBranch(schema) {
  return schemaBranches(schema).find((branch) => branch.type === "integer") || {};
}
function slugifyAdobeModel(modelId, modelVersion) {
  const slug = (value, allowDot = false) => String(value || "").trim().toLowerCase().replace(allowDot ? /[^a-z0-9.]+/g : /[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  const family = slug(modelId);
  const publicVersion = family === "kling" ? modelVersion.replace(/^kling_v3_omni/i, "kling_o3") : modelVersion;
...
function parseAdobeModelsDiscovery(body) {
  ...
  for (const familyValue of families) {
    ...
    for (const [modelVersion, versionValue] of Object.entries(asRecord(family.modelVersions))) {
      ...
      const outputModalities = asStringArray(version.outputModality).map(
        (item) => item.toLowerCase()
      );
      const modality = outputModalities.includes("image") ? "image" : outputModalities.includes("video") ? "video" : outputModalities.includes("audio") ? "audio" : "unknown";
      rows.push({
        modelId,
        modelVersion,
        displayName: String(
        ...
        providerName: typeof family.acModelFamilyProviderDisplayName === "string" ? family.acModelFamilyProviderDisplayName : void 0,
        releaseReadiness: typeof version.releaseReadiness === "string" ? version.releaseReadiness : void 0,
        healthStatus: typeof version.healthStatus === "string" ? version.healthStatus : void 0,
        inputMediaUseCases: asStringArray(version.inputMediaUseCase),
        requestSchema: asRecord(version.requestSchema),
        backingModel: typeof version.bksGenerationModel === "string" ? version.bksGenerationModel : void 0
      });
    }
  }
  return rows;
...
function normalizeCapabilities(row) {
  ...
  const mediaCapabilities = Array.isArray(referenceSchema["x-capabilities"]) ? referenceSchema["x-capabilities"] : [];
  ...
  const supportedSizes = [
    ...new Set(
      schemaBranches(schema.properties.size).flatMap((branch) => Array.isArray(branch.enum) ? branch.enum : []).map(asRecord).filter((size) => finiteInteger(size.width) !== null && finiteInteger(size.height) !== null).map((size) => `${size.width}x${size.height}`)
    )
  ];
  const supportedAspectRatios = [
    ...new Set(
      schemaBranches(schema.properties.generationSettings).flatMap(
        (branch) => enumStrings(asRecord(asRecord(branch.properties).aspectRatio))
      )
    )
  ];
  const duration = integerBranch(schema.properties.duration);
  const outputCount = integerBranch(schema.properties.n);
  const prompt = schemaBranches(schema.properties.prompt).find((branch) => branch.type === "string") || {};
  return {
    ...
    supportedDurations: [
      ...new Set(
        schemaBranches(schema.properties.duration).flatMap((branch) => Array.isArray(branch.enum) ? branch.enum : []).filter((value) => Number.isInteger(value))
      )
    ],
...
function isCallableGenerationModel(row) {
...
function mapDiscoveredToCatalog(rows) {
  const output = [];
...
const ADOBE_FIREFLY_FALLBACK_MODELS = snapshotCatalog();
function getAdobeFireflyFallbackCatalog(modality) {
  return ADOBE_FIREFLY_FALLBACK_MODELS.filter((model) => !modality || model.modality === modality);
}
function imageFamily(model) {
...
const ADOBE_FIREFLY_IMAGE_MODELS = Object.fromEntries(
  getAdobeFireflyFallbackCatalog("image").map((model) => [
    model.id,
    { ...model, modality: "image", family: imageFamily(model) }
  ])
);
...
const ADOBE_FIREFLY_VIDEO_MODELS = Object.fromEntries(
...
const ADOBE_FIREFLY_IMAGE_ROUTING_ALIASES = Object.freeze(
  Object.entries(LEGACY_MODEL_ALIASES).filter(([, target]) => Boolean(ADOBE_FIREFLY_IMAGE_MODELS[target])).map(([alias]) => alias)
);
function normalizeRequestedId(model) {
  return String(model || "").trim().toLowerCase().replace(/^adobe-firefly\//, "").replace(/^firefly\//, "");
}
function resolveCatalogId(model) {
...
function toRegistryImageModels() {
...
function toRegistryVideoModels() {
...
function toAdobeMediaCapabilitiesApi(model) {
  const caps = model.capabilities;
  return {
...
function getAdobeReferenceUploadLimit(model, mediaType) {
  if (model.capabilities.maxReferenceItems !== null) {
    return Math.max(1, Math.min(32, model.capabilities.maxReferenceItems));
  }
  const declaredTotal = model.capabilities.referenceInputs.filter((reference) => reference.mediaType === mediaType).reduce((total, reference) => total + (reference.maxItems ?? 0), 0);
...
export {
  ...
  getAdobeReferenceUploadLimit,
  mapDiscoveredToCatalog,
  mergeAdobeObjectSchema,
