...
export function reorderByCapabilities(models, required) {
  ...
  const alreadySorted = indexed.every((x, i) => i === 0 || indexed[i - 1].t <= x.t);
  if (alreadySorted) return models; // unchanged → keep reference
  return indexed
...
function reorderByCost(models) {
  if (!Array.isArray(models) || models.length <= 1) return models;
  const slash = (m) => { const i = m.indexOf("/"); return i > 0 ? m.slice(0, i) : ""; };
  return [...models].sort((a, b) => {
    const costA = PROVIDER_COST_TIERS[slash(a)] ?? 99;
    const costB = PROVIDER_COST_TIERS[slash(b)] ?? 99;
    if (costA !== costB) return costA - costB;
    // Same cost tier: maintain original order
...
export function detectRequiredCapabilities(body) {
  ...
  // search: request-wide (lives in tools, not the user turn). web_search is
  // OpenAI's built-in search tool; a server-side tool declared by the client
  // also implies the model should support grounding.
  const tools = Array.isArray(body.tools) ? body.tools : [];
  const searchTool = tools.some((t) =>
    (t?.type === "web_search" || t?.type === "web_search_preview")
    || (typeof t?.function?.name === "string" && t.function.name.toLowerCase().includes("search"))
  );
  if (searchTool) required.add("search");
 ...
 * @param {string} strategy - "fallback" or "round-robin" or "cost-optimized"
 * @param {number|string} [stickyLimit=1] - Requests per combo model before switching
 * @returns {string[]} Rotated models array
 */
export function getRotatedModels(models, comboName, strategy, stickyLimit = 1) {
  if (!models || models.length <= 1) {
    return strategy === "cost-optimized" && models.length > 1 ? reorderByCost(models) : models;
  }

  // Cost-optimized: reorder by cost tier (cheapest first), then use fallback
