...
export function filterToOpenAIFormat(body, opts = {}) {
  ...
  body.messages = body.messages.map(msg => {
    ...
    if (Array.isArray(msg.content)) {
      ...
      // Text-only arrays collapse to a plain string (accepted by all OpenAI
      // compatible providers); multimodal arrays stay as content blocks.
      // Never flatten when cache_control markers must survive (alicode/DashScope).
      const textOnly = filteredContent.every((b) => b?.type === OPENAI_BLOCK.TEXT);
      const hasCacheControl = filteredContent.some((b) => b?.cache_control != null);
      if (textOnly && !hasCacheControl) {
