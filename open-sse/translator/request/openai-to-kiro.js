 ...
 *
 * Two oryphemrouter-specific behaviours implemented here:
 *
...
export function openaiToKiroRequest(model, body, stream, credentials) {
  ...
  if (!canonical.valid) {
    engineLog.error("KIRO", sanitize(`refusing invalid conversation (openai → kiro): ${(canonical.errors || []).join(", ") || "unknown"} | turns=${(canonical.history || []).length + 1}`));
    return null;
  }
