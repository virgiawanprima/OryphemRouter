...
export function parseSSELine(line, format = null) {
  ...
  // Auto-detected when the line starts with '{' — SSE data lines always start
  // with "data:", so a leading '{' is unambiguous (no format arg required).
  const trimmed0 = line.trim();
  if (trimmed0.startsWith("{")) {
    try {
      return JSON.parse(trimmed0);
    } catch (error) {
      return null;
    }
  }
  ...
  try {
    ...
    log.warn("SSE", `Failed to parse SSE line (${data.length} chars): ${sanitize(data.substring(0, 200))}...`);
    return null;
  }
}
