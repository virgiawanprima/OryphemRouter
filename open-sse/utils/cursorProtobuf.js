...
// agent.v1 / Mcp* codec used by the executor's AgentService path. Field numbers
// verified against Cursor's agent.proto (extracted via @oh-my-pi).
...
function decodeDouble(bytes) {
  const view = bytes.buffer instanceof ArrayBuffer ? bytes.buffer : bytes;
  return new DataView(view, bytes.byteOffset || 0, 8).getFloat64(0, false);
}
...
export function encodeAgentValue(value) {
  ...
  if (typeof value === "number") {
    // double number_value (FIXED64)
    const tag = encodeVarint((2 << 3) | WIRE_TYPE.FIXED64);
    return concatArrays(tag, encodeDouble(value));
  }
  if (Array.isArray(value)) {
    // list_value: repeated Value values = 1
    const inner = concatArrays(...value.map((v) => encodeField(1, WIRE_TYPE.LEN, encodeAgentValue(v))));
    return encodeField(6, WIRE_TYPE.LEN, inner);
  }
  if (typeof value === "object") {
    // struct_value: map<string, Value> fields = 1 (entry: field1 key, field2 Value)
    ...
    const inner = concatArrays(...entries);
    return encodeField(5, WIRE_TYPE.LEN, inner);
  }
...
export function decodeAgentValue(bytes) {
  ...
  if (fields.has(1)) return null;                        // null_value
  if (fields.has(2)) return decodeDouble(fields.get(2)[0].value); // number_value
  if (fields.has(3)) return textDecoder.decode(fields.get(3)[0].value); // string_value
  if (fields.has(4)) return fields.get(4)[0].value !== 0; // bool_value
  if (fields.has(6)) {                                   // list_value
    return (fields.get(6)[0].value
      ? decodeMessage(fields.get(6)[0].value).get(1) || []
      : []).map((f) => decodeAgentValue(f.value));
  }
  if (fields.has(5)) {                                   // struct_value
    const struct = decodeMessage(fields.get(5)[0].value);
    const out = {};
    for (const entry of struct.get(1) || []) {
      const ef = decodeMessage(entry.value);
      const key = textDecoder.decode(ef.get(1)?.[0]?.value || new Uint8Array(0));
      out[key] = decodeAgentValue(ef.get(2)?.[0]?.value);
    }
    return out;
  }
  return null;
...
export function encodeMcpToolDefinition(tool) {
  ...
  const name = t.name || t.functionName || "";
  ...
  const parts = [
    ...
    encodeField(3, WIRE_TYPE.LEN, encodeAgentValue(schema)), // input_schema (Value)
    encodeField(4, WIRE_TYPE.LEN, "oryphemrouter"),               // provider
    name ? encodeField(5, WIRE_TYPE.LEN, name) : new Uint8Array(0), // tool_name
  ];
...
export function decodeMcpArgs(bytes) {
  ...
  for (const entry of fields.get(2) || []) {
    const ef = decodeMessage(entry.value);
    const key = textDecoder.decode(ef.get(1)?.[0]?.value || new Uint8Array(0));
    args[key] = decodeAgentValue(ef.get(2)?.[0]?.value);
  }
  const str = (field) => {
    const f = fields.get(field)?.[0];
    return f ? textDecoder.decode(f.value) : "";
  };
  return { name: str(1), toolCallId: str(3), toolName: str(5), args };
}

// ─── McpResult ───────────────────────────────────────────────────────────────
// McpResult { oneof: success(1), error(2), toolNotFound(5) }.
// success: { repeated content(1), bool is_error(2) }
//   text item:  { TextContent(1) { text(1) } }
//   image item: { ImageContent(2) { data(1), mimeType(2) } }

function encodeTextItem(text) {
  return encodeField(1, WIRE_TYPE.LEN, encodeField(1, WIRE_TYPE.LEN, String(text)));
}

function encodeImageItem({ data, mimeType }) {
  const raw = data instanceof Uint8Array ? data : Buffer.from(data || []);
  return encodeField(2, WIRE_TYPE.LEN, concatArrays(
    encodeField(1, WIRE_TYPE.LEN, raw),
    encodeField(2, WIRE_TYPE.LEN, String(mimeType || "")),
  ));
}

export function encodeMcpResultSuccess({ textItems = [], imageItems = [], isError = false } = {}) {
  const items = [
    ...(textItems || []).map(encodeTextItem),
    ...(imageItems || []).map(encodeImageItem),
  ];
  const success = concatArrays(
    ...items.map((it) => encodeField(1, WIRE_TYPE.LEN, it)),
    encodeField(2, WIRE_TYPE.VARINT, isError ? 1 : 0),
  );
  return encodeField(1, WIRE_TYPE.LEN, success); // success variant
}

export function encodeMcpResultError(message) {
  const err = encodeField(1, WIRE_TYPE.LEN, String(message || ""));
  return encodeField(2, WIRE_TYPE.LEN, err); // error variant
}

export function encodeMcpResultToolNotFound(toolName) {
  const tnf = encodeField(1, WIRE_TYPE.LEN, String(toolName || ""));
  return encodeField(5, WIRE_TYPE.LEN, tnf); // toolNotFound variant
}
