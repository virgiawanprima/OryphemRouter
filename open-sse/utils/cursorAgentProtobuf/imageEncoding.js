...
const DIM_WIDTH = 1;
const DIM_HEIGHT = 2;
function cursorImageAttachmentPath(uuid, mimeType) {
  const normalized = (mimeType || "").toLowerCase();
  const ext = normalized === "image/jpeg" || normalized === "image/jpg" ? "jpg" : normalized === "image/gif" ? "gif" : normalized === "image/webp" ? "webp" : "png";
  return `attachment-${uuid}.${ext}`;
}
function encodeSelectedImageBody(img, blobStore) {
