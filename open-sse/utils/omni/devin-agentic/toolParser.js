...
function validateSchema(value, schema, path) {
  ...
  if (Array.isArray(value) && schema.items) {
    ...
    value.forEach(
      (item, index) => errors.push(...validateSchema(item, itemSchema, `${path}[${index}]`))
    );
  }
  return errors;
}
function parseDevinToolRequest(text, tools, idSeed = "") {
  ...
  const digest = createHash("sha256").update(`${idSeed}:${name}:${stableJson(input)}`).digest("hex").slice(0, 16);
