...
// Subcommands (`oryphemrouter xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.oryphemrouter/runtime
...
// Identifiers for killAllAppProcesses - only kill oryphemrouter specifically
const PROCESS_IDENTIFIERS = [
  'oryphemrouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "oryphemrouter")
    : path.join(os.homedir(), ".oryphemrouter");
}
...
// Kill all oryphemrouter processes
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running oryphemrouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "oryphemrouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("oryphemrouter") && (cmd.includes("cli.js") || cmd.includes("\\oryphemrouter") || cmd.includes("/oryphemrouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running oryphemrouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "oryphemrouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("oryphemrouter") && (cmd.includes("cli.js") || cmd.includes("/oryphemrouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 oryphremrouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 oryphremrouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "oryphemrouter", "db.json") : path.join(".oryphemrouter", "db.json"));
