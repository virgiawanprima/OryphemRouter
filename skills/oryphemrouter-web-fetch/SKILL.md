---
name: oryphemrouter-web-fetch
description: Fetch URL → markdown / text / HTML via oryphremrouter /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# oryphremrouter — Web Fetch
...
```bash
curl $NINE_ROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$NINE_ROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $NINE_ROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $NINE_ROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $NINE_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://oryphemrouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.NINE_ROUTER_URL}/v1/web/fetch`, {
  method: "POST",
