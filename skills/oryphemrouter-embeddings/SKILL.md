---
name: oryphemrouter-embeddings
description: Generate vector embeddings via oryphremrouter /v1/embeddings using OpenAI / Gemini / Mistral / Voyage / Nvidia / GitHub embedding models for RAG, semantic search, similarity. Use when the user wants embeddings, vectors, RAG, semantic search, or to embed text.
---

# oryphremrouter — Embeddings
...
```bash
curl $NINE_ROUTER_URL/v1/models/embedding | jq '.data[].id'
# Per-model dimensions
curl "$NINE_ROUTER_URL/v1/models/info?id=openai/text-embedding-3-small"
```

## Endpoint

`POST $NINE_ROUTER_URL/v1/embeddings`
...
```bash
curl -X POST $NINE_ROUTER_URL/v1/embeddings \
...
```js
const r = await fetch(`${process.env.NINE_ROUTER_URL}/v1/embeddings`, {
  method: "POST",
