---
name: oryphemrouter-chat
description: Chat / code generation via oryphremrouter using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through oryphremrouter.
---
...
- `POST $NINE_ROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $NINE_ROUTER_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $NINE_ROUTER_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$NINE_ROUTER_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $NINE_ROUTER_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.NINE_ROUTER_URL}/v1`, apiKey: process.env.NINE_ROUTER_KEY });
...
```bash
curl -X POST $NINE_ROUTER_URL/v1/messages \
  -H "Authorization: Bearer $NINE_ROUTER_KEY" \
